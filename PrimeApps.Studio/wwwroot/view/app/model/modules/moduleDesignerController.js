    ...
    .controller('moduleDesignerController', ['$rootScope', '$scope', '$filter', '$location', '$state', '$q', '$popover', '$modal', 'helper', '$timeout', 'dragularService', 'defaultLabels', '$interval', '$cache', 'systemRequiredFields', 'systemReadonlyFields', 'ModuleService', 'LayoutService', '$element', 'operators',
        function ($rootScope, $scope, $filter, $location, $state, $q, $popover, $modal, helper, $timeout, dragularService, defaultLabels, $interval, $cache, systemRequiredFields, systemReadonlyFields, ModuleService, LayoutService, $element, operators) {
            ...
            $scope.lookupTypeChanged = function () {
                ...
                if (lookupModule) {
                    ...
                    switch (lookupModuleName) {
                        case 'users':
                            $scope.currentLookupFields = ModuleService.getFieldsOperator(ModuleService.getFakeUserModule()).fields;
                            break;
                        case 'roles':
                            $scope.currentLookupFields = ModuleService.getFieldsOperator(ModuleService.getFakeRoleModule()).fields;
                            break;
                        case 'profiles':
                            $scope.currentLookupFields = ModuleService.getFieldsOperator(ModuleService.getFakeProfileModule()).fields;
                            break;
                        default:
                            break;
                    }
            ...
            $scope.selectFieldToFilter = function (filter, selectedField, param) {
                ...
                switch (lookType) {
                    case 'users':
                        filter[param] = ModuleService.getFakeUserModule();
                        break;
                    case 'roles':
                        filter[param] = ModuleService.getFakeRoleModule();
                        break;
                    case 'profiles':
                        filter[param] = ModuleService.getFakeProfileModule();
                        break;
                    default:
