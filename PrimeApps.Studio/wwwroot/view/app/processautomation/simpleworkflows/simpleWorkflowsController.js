...
angular.module('primeapps')
    ...
    .controller('SimpleWorkflowsController', ['$rootScope', '$scope', '$filter', '$state', '$stateParams', '$modal', '$timeout', 'helper', 'dragularService', 'operators', 'SimpleWorkflowsService', 'ModuleService', 'LayoutService', '$http', 'config', '$localStorage', '$cookies', 'FileUploader',
        function ($rootScope, $scope, $filter, $state, $stateParams, $modal, $timeout, helper, dragularService, operators, SimpleWorkflowsService, ModuleService, LayoutService, $http, config, $localStorage, $cookies, FileUploader) {
            ...
            $scope.$parent.activeMenuItem = 'simpleworkflows';
            $scope.workflowModel = {};
            ...
            $scope.environments = SimpleWorkflowsService.getEnvironments();
 ...
 */
            ...
            $scope.validate = function (tabClick, previous) {
                if (!tabClick)
                ...
                if (tabClick.module.$error.required) {
                    // toastr.error($filter('translate')('Module.RequiredError'));
                    return false;
                } else if (tabClick.$error.operations) {
                    /// toastr.error($filter('translate')('Setup.Workflow.OperationRequiredError'));
                    return false;
                }
                ...
                if ($scope.workflowModel.changed_field_checkbox && !tabClick.changed_field.$valid)
                    return false;
                //TODO 
                return $scope.validateActions(tabClick, previous);
            };
            ...
            $scope.sendNotificationIsNullOrEmpty = function (workflowForm) {
                ...
                if (workflowForm.$error) {
                    if (workflowForm.$error.required || workflowForm.$error.minTags) {
                        //toastr.error($filter('translate')('Module.RequiredError'));
                        return false;
                    }
                }
            ...
            $scope.validateActions = function (tabClick, previous) {
                if (!$scope.lastStepClicked && !tabClick) {
                    tabClick.$submitted = false;
                    return true;
                }
                var sendNotificationIsNullOrEmpty = $scope.sendNotificationIsNullOrEmpty(tabClick);
                ...
                if (sendNotificationIsNullOrEmpty && createTaskIsNullOrEmpty && fieldUpdateIsNullOrEmpty && webHookIsNullOrEmpty) {
                    if (tabClick) {
                        tabClick.$submitted = false;
                        if ($scope.$parent.wizardStep === 2 && !previous)
                            return false;
            ...
            $scope.save = function (workflowForm) {
                ...
                angular.forEach($scope.environments, function (env) {
                    if (env.selected)
                        workflow.environments.push(env.value);
                });

                delete workflow.environment;
                delete workflow.environment_list;
            ...
            $scope.showFormModal = function (id) {
                ...
                $scope.ruleFormModal = $scope.ruleFormModal || $modal({
                    scope: $scope,
                    templateUrl: 'view/app/processautomation/simpleworkflows/simpleWorkflowModal.html',
            ...
            $scope.nextStep = function (form, stepId) {
                ...
                if (result) {
                    ...
                    // form.methodType.$setValidity('required', false);
                    return false;
                }
            ...
            $scope.previousStep = function (form, stepId) {
                var result = $scope.validate(form, true);
                if (result)
                    if (stepId > 0) {
                        $scope.$parent.wizardStep = stepId - 1;
            ...
            $scope.tabControl = function (workflowForm, tab) {
                if ($scope.wizardStep !== 2) {
                    workflowForm.$submitted = false;
                    //toastr.error($filter('translate')('Module.RequiredError'));
                    return true;
                }

                if (tab == 0) {
                    var sendNotificationIsNullOrEmpty = $scope.sendNotificationIsNullOrEmpty(workflowForm);
                    var isValideNotification = $scope.validateSendNotification();
                    if (sendNotificationIsNullOrEmpty)
                        return true;
                    else if (!sendNotificationIsNullOrEmpty && isValideNotification)
                        return true;
                    else {
                    ...
                    var fieldUpdateIsNullOrEmpty = $scope.fieldUpdateIsNullOrEmpty();
                    var isValidateUpdateField = $scope.validateUpdateField();

                    if (fieldUpdateIsNullOrEmpty || !$scope.workflowModel.field_update.updateOption)
                        return true;
                    else if (!fieldUpdateIsNullOrEmpty && isValidateUpdateField)
                        return true;
                    else {
                        workflowForm.$submitted = true;
                        toastr.error($filter('translate')('Module.RequiredError'));
                    ...
                    var webHookIsNullOrEmpty = $scope.webHookIsNullOrEmpty();
                    var isValidateWebhook = $scope.validateWebHook();

                    if (webHookIsNullOrEmpty)
                        return true;
                    else if (!webHookIsNullOrEmpty && isValidateWebhook)
                        return true;
                    else {
                        workflowForm.$submitted = true;
                        toastr.error($filter('translate')('Module.RequiredError'));
                ...
                //if ((sendNotificationIsNullOrEmpty && fieldUpdateIsNullOrEmpty && webHookIsNullOrEmpty) ||
                //    ((!sendNotificationIsNullOrEmpty || !fieldUpdateIsNullOrEmpty || !webHookIsNullOrEmpty) || (isValideNotification || isValidateUpdateField || isValidateWebhook)))
                //if ((!sendNotificationIsNullOrEmpty && isValideNotification) && (!fieldUpdateIsNullOrEmpty && isValidateUpdateField) && (webHookIsNullOrEmpty && isValidateWebhook))
                //    return true;
                //else {
                workflowForm.$submitted = true;
                toastr.error($filter('translate')('Module.RequiredError'));
