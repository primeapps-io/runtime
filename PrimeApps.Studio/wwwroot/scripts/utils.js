...
angular.module('primeapps')
    ...
    .factory('helper', ['$rootScope', '$timeout', '$filter', '$localStorage', '$sessionStorage', '$q', '$http', 'config', '$cache', 'appMenus',
        function ($rootScope, $timeout, $filter, $localStorage, $sessionStorage, $q, $http, config, $cache, appMenus) {
            return {
                ...
                checkUserProfile: function (menu, page, operation) {
                    ...
                    var userRole = $rootScope.currentApp.user_profile;
                    ...
                    switch (userRole) {
                        case 'manager':
                            ...
                            break;
                        case 'developer':
                            if (menu === 'manage' || appMenus['manage'].indexOf(page) > -1)
                                return false;
                            else
                                return true;
                            break;
                        case 'viewer':
                            if (menu != 'overview' || (operation === 'update' || operation === 'create' || operation === 'delete'))
                                return false;
                            else if (menu === 'overview' || operation === 'read')
                                return true;
                            break;
                        case 'tenant_admin':
                            return true;
