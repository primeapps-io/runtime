...
namespace PrimeApps.Studio.Helpers
{
    ...
    public class WorkflowHelper : IWorkflowHelper
    {
            ...
            using (var _scope = _serviceScopeFactory.CreateScope())
            {
                ...
                using (var _settingRepository = new SettingRepository(databaseContext, _configuration))
                {
                    ...
                    foreach (var workflow in workflows)
                    {
                        ...
                        record = await _recordRepository.GetById(module, (int)record["id"], false, lookupModules, true, profileBasedEnabled: false);
                        ...
                        if (workflow.SendNotification != null)
                        {
                            ...
                            //TODO var email = new Email(EmailResource.WorkflowNotification, appUser.Culture, emailData, _configuration, _serviceScopeFactory, appUser.AppId, appUser);
                            ...
                            foreach (var recipientItem in recipients)
                            {
                                ...
                                //TODO  email.AddRecipient(recipient);

                            }

                            //TODOif (sendNotification.Schedule.HasValue)
                            //TODO   email.SendOn = DateTime.UtcNow.AddDays(sendNotification.Schedule.Value);

                            //TODOemail.AddToQueue(appUser.TenantId, module.Id, (int)record["id"], "", "", sendNotificationCC, sendNotificationBCC, appUser: appUser, addRecordSummary: false);
        ...
        {
            if (workflowModel.Environments == null)
            {
                workflowModel.Environments = new List<EnvironmentType>()
                {
                    EnvironmentType.Development
                };
            }
            else if (workflowModel.Environments.Count < 1)
                workflowModel.Environments.Add(EnvironmentType.Development);
            ...
            var workflow = new Workflow
            {
                ...
                Environment = workflowModel.EnvironmentValues
            };
        ...
        public async Task UpdateEntity(WorkflowBindingModel workflowModel, Workflow workflow, string tenantLanguage)
        {
            ...
            workflow.Environment = workflowModel.EnvironmentValues;
