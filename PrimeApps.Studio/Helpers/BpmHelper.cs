...
namespace PrimeApps.Studio.Helpers
{
    ...
    public class BpmHelper : IBpmHelper
    {
        ...
        //TODO	private IWorkflowHost _workflowHost;
        //TODO private IWorkflowRegistry _workflowRegistry;
        //TODO private IPersistenceProvider _workflowStore;
        //TODO private IDefinitionLoader _definitionLoader;
        private IHttpContextAccessor _context;
        private IConfiguration _configuration;
        private IServiceScopeFactory _serviceScopeFactory;

        public BpmHelper(IServiceScopeFactory serviceScopeFactory, IHttpContextAccessor context, IConfiguration configuration)//TODO, IWorkflowHost workflowHost, IWorkflowRegistry workflowRegistry, IPersistenceProvider workflowStore, IDefinitionLoader definitionLoader)
        {
            //TODO	_workflowHost = workflowHost;
            //TODO	_workflowRegistry = workflowRegistry;
            //TODO	_workflowStore = workflowStore;
            //TODO	_definitionLoader = definitionLoader;
        ...
        public async Task UpdateEntity(BpmWorkflowBindingModel bpmWorkflowModel, BpmWorkflow bpmWorkflow, string tenantLanguage)
        {
            ...
            bpmWorkflow.Active = bpmWorkflowModel.Active;
            ...
            using (var _scope = _serviceScopeFactory.CreateScope())
            {
                ...
                using (var _picklistRepository = new PicklistRepository(databaseContext, _configuration))
                {
                    ...
                    foreach (var workflow in bpmWorkflows)
                    {
                        ...
                        try
                        {
                            //TODO runId = await _workflowHost.StartWorkflow<JObject>(code, data, referance);
                            var resultCreateLog = await _BpmWorkflowRepository.CreateLog(workflowLog);

                            if (resultCreateLog < 1)
                                ErrorHandler.LogError(new Exception("WorkflowLog cannot be created! workflowLogObject: " + workflowLog), "WorkflowId: " + workflow.Id + " " + "tenant_id:" + appUser.TenantId + "ModuleId:" + module.Id + "operation_type:" + operationType + " record_id:" + record["id"].ToString());
                        }
                        catch (Exception ex)
                        {
                            ErrorHandler.LogError(ex, $"WorkflowLog tenant {_currentUser.TenantId} workflowLog:" + workflowLog + " " + "tenant_id:" + _currentUser.TenantId + "  runId:" + runId + "WorkflowId: " + workflow.Id + "ModuleId:" + module.Id + " record_id:" + record["id"].ToString());
                        }
                    }
                }
            }
