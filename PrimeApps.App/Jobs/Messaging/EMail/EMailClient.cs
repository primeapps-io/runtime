...
namespace PrimeApps.App.Jobs.Messaging.EMail
{
    ...
    class EMailClient : MessageClient
    {
                ...
                using (var tenantDBContext = new TenantDBContext(emailQueueItem.TenantId))
                {
                    ...
                    using (EMailProvider emailClient = EMailProvider.Initialize(provider, userName, password))
                    {
                        ...
                        if (bulkEMailStatus == NotificationStatus.Successful)
                        {
                            ...
                            composerResult = await Compose(emailQueueItem, emailTemplate, module, emailField, subject, query, ids, isAllSelected, emailNotification.CreatedById, language, emailId, tenantDBContext, platformUserRepository, emailClient, emailNotification.AttachmentLink, emailNotification.AttachmentName);
        ...
        public async Task<EMailComposerResult> Compose(MessageDTO messageDto, string messageBody, Module module, string emailField, string subject, string query, string[] ids, bool isAllSelected, int userId, string language, string emailId, TenantDBContext dbContext, PlatformUserRepository platformUserRepository, EMailProvider emailClient, string attachmentLink, string attachmentName)
        {
            ...
            var subscriber = await platformUserRepository.GetWithTenant(messageDto.TenantId);
                ...
                if (ids?.Length > 0)
                {
                    ...
                    using (var recordRepository = new RecordRepository(databaseContext))
                    {
                        foreach (string recordId in ids)
                        {
                            ...
                            record = await Model.Helpers.RecordHelper.FormatRecordValues(module, record, moduleRepository, picklistRepository, subscriber.Tenant.Language, subscriber.Culture, 180, lookupModules, true);
