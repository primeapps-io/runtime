...
namespace PrimeApps.App.Bpm.Steps
{
    public class NotificationStep : StepBodyAsync
    {
        ...
        public override async Task<ExecutionResult> RunAsync(IStepExecutionContext context)
        {
                ...
                using (var _scope = _serviceScopeFactory.CreateScope())
                {
                    ...
                    using (var _analyticRepository = new AnalyticRepository(databaseContext, _configuration))
                    {
                        ...
                        using (var _userRepository = new UserRepository(databaseContext, _configuration))
                        {
                            ...
                            await email.AddToQueue(appUser.TenantId, module.Id, (int)record["id"], "", "", sendNotificationCC == null ? "" : string.Join(",", sendNotificationCC.ToObject<string[]>()), sendNotificationBCC == null ? "" : string.Join(",", sendNotificationBCC.ToObject<string[]>()), appUser: appUser, addRecordSummary: false);
            ...
            {
                ErrorHandler.LogError(ex, $"Notifacition Step Error");
                return ExecutionResult.Next();
            }
        }
    }
