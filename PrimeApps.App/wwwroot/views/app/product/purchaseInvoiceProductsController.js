        ...
        function ($rootScope, $scope, $state, config, ngToast, $localStorage, $filter, ngTableParams, $stateParams, helper, QuoteProductsService, ModuleService, $popover) {
            ...
            $scope.lookup = function (searchTerm) {
                if (isExtraField) {
                    for (var i = 0; extraadditionalFields.length > i; i++) {
                        var field = $filter('filter')($scope.purchaseInvoiceProductModule.fields, { name: extraadditionalFields[i] }, true);
                        if (field.length > 0) {
            ...
            $scope.selectProduct = function (purchaseInvoiceProduct) {
                ...
                if (!purchaseInvoiceProduct.defaultCurrency) {
                    purchaseInvoiceProduct.defaultCurrency = purchaseInvoiceProduct.product.currency.value;
                    purchaseInvoiceProduct.currencyConvertList = $scope.currencyConvert(purchaseInvoiceProduct, purchaseInvoiceProduct.product.unit_price);
                } else {
                    purchaseInvoiceProduct.product.unit_price = purchaseInvoiceProduct.currencyConvertList[purchaseInvoiceProduct.product.currency.value];
                }

                var unitPrice = parseFloat(purchaseInvoiceProduct.product.unit_price);

                if (!purchaseInvoiceProduct.quantity)
                    purchaseInvoiceProduct.quantity = 1;
                purchaseInvoiceProduct.unit_price = unitPrice;
                purchaseInvoiceProduct.amount = unitPrice;
            ...
            $scope.calculate = function (purchaseInvoiceProduct) {
                ...
                if (!isNaN(purchaseInvoiceProduct.unit_price))
                    unitPrice = angular.copy(purchaseInvoiceProduct.unit_price);
                ...
                purchaseInvoiceProduct.amount = quantity * unitPrice;
                switch (purchaseInvoiceProduct.discount_type) {
                    case 'percent':
                        ...
                        purchaseInvoiceProduct.discount_amount = ((unitPrice * quantity) * purchaseInvoiceProduct.discount_percent) / 100;
                    ...
                    case 'amount':
                        if (purchaseInvoiceProduct.discount_amount != undefined && purchaseInvoiceProduct.discount_amount != null && !isNaN(purchaseInvoiceProduct.discount_amount)) {
                            ...
                            purchaseInvoiceProduct.discount_percent = (100 / (unitPrice * quantity)) * purchaseInvoiceProduct.discount_amount;
                        }
                        if ($scope.fields['unit_amount']) {
                            purchaseInvoiceProduct.unit_amount = purchaseInvoiceProduct.amount / quantity;
                        }
            ...
            $scope.calculateAll = function () {
                ...
                angular.forEach($scope.$parent.$parent.purchaseInvoiceProducts, function (purchaseInvoiceProduct) {
                    ...
                    if (purchaseInvoiceProduct.unit_price < 0) {
                        purchaseInvoiceProduct.unit_price = 0;
                    }

                    if (purchaseInvoiceProduct.unit_price > 1000000000000) {
                        purchaseInvoiceProduct.unit_price = 1000000000000;
                    }
