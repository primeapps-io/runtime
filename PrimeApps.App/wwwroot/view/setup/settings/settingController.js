				...
				function editUser() {
					...
					SettingService.editUser(userModel)
						.then(function () {
							...
							if (!emailChanged)
								ngToast.create({
									content: $filter('translate')('Setup.Settings.UpdateSuccess'),
									className: 'success'
								});
							else
								ngToast.create({
									content: $filter('translate')('Setup.Settings.UpdateSuccessEmail'),
									className: 'success',
									timeout: 6000
								});
				...
				if ($scope.passwordForm.$valid) {
					...
					SettingService.changePassword(passwordModel.current, passwordModel.password, passwordModel.confirm)
						.then(function () {
							...
							ngToast.create({
								content: $filter('translate')('Setup.Settings.PasswordSuccess'),
								className: 'success'
							});
						})
			...
			$scope.changeCurrency = function (code) {
				SettingService.changeCurrency(code)
					.then(function () {
						...
						ngToast.create({
							content: $filter('translate')('Setup.Settings.CurrencySuccess'),
							className: 'success'
						});
					});
			};
			...
			$scope.deleteAccount = function (confirmPassword) {
				...
				SettingService.removeUser(confirmPassword)
					...
					.catch(function (response) {
						if (response.status === 400) {
							ngToast.create({
								content: $filter('translate')('Setup.Settings.InvalidPassword'),
								className: 'danger',
								timeout: 6000
							});
						}
