			...
			$scope.save = function (menu) {
				...
				if ($scope.id && !clone) {
					...
					var updateList = {
						...
						profileId: $scope.defaultMenu ? $scope.profileItem.id : systemSubscriberProfileId
					};
					//we will check  first values for update
					var menuIsUpdate = false;
					...
					if ($scope.updateArray.length > 0 && menuIsUpdate)
						MenuService.update($scope.id, $scope.updateArray);
				...
				//If first create
					...
					MenuService.create(menu).then(function () {
						MenuService.createMenuItems($scope.menuLists, menu[0].profileId).then(function onSuccess() {
