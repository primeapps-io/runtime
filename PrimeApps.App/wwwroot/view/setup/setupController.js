...
angular.module('primeapps')
    ...
    .controller('SetupController', ['$rootScope', '$scope', '$filter', '$location', 'helper',
        function ($rootScope, $scope, $filter, $location, helper) {
            ...
            kendo.ui.Grid.prototype.options.filterMenuInit = function (e) {
                var firstValueDropDown = e.container.find("select:eq(0)").data("kendoDropDownList");
                var firstFilter = null;
                if (firstValueDropDown && firstValueDropDown.dataSource)
                    firstFilter = $filter('filter')(firstValueDropDown.dataSource.options.data, {value: 'contains'}, true)[0];
                if (firstFilter){
                    firstValueDropDown.value("contains");
                    firstValueDropDown.trigger("change");

                    var secondValueDropDown = e.container.find("select:eq(2)").data("kendoDropDownList");
                    if (secondValueDropDown && secondValueDropDown.dataSource){
