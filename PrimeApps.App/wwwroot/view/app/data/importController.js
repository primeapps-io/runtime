						...
						angular.forEach($scope.headerRow, function (value, key) {
							...
							if ($rootScope.language === 'tr') {
								...
								angular.forEach($scope.module.fields, function (field) {
									...
									if (field.name === 'owner') {
										used = true;
										$scope.fieldMap[field.name] = 'currentUser';
									}
									else if (field['label_' + $rootScope.language].toLowerCase() === value.trim().toLowerCase()) {
										used = true;
										$scope.fieldMap[field.name] = key;
									}
									else {
				...
				for (var i = 0; i < $scope.rows.length; i++) {
					...
					if (!row[$scope.fieldMap["owner"]]) {
						...
						var isRowOwnerRight = $filter('filter')($scope.users, function (user) {
							return user.full_name.trim().toLowerCaseTurkish() === row[$scope.fieldMap["owner"]].toString().trim().toLowerCaseTurkish();
						}, true)[0];
