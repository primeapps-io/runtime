	...
	.controller('ModuleListController', ['$rootScope', '$scope', 'ngToast', '$sce', '$filter', 'helper', '$location', '$state', '$stateParams', '$q', '$window', '$localStorage', '$cache', 'config', 'ngTableParams', 'blockUI', 'exportFile', '$popover', '$modal', 'operations', 'activityTypes', 'transactionTypes', 'ModuleService', '$http', 'components',
		function ($rootScope, $scope, ngToast, $sce, $filter, helper, $location, $state, $stateParams, $q, $window, $localStorage, $cache, config, ngTableParams, blockUI, exportFile, $popover, $modal, operations, activityTypes, transactionTypes, ModuleService, $http, components) {
			...
			$scope.selectAll = function ($event, data) {
				/**Burayı comente alma sebebimiz
				 * Liste ekranlarında kayıt seçimi yapılıp sayfalama işlemiyle sonraki sayfaya gidildiğinde 
				 * önceki sayfada seçilen kayıtların seçimi kalkmaktaydı.*/
				// $scope.selectedRows = [];
				var index = -1;

				if ($scope.isAllSelected) {
					$scope.isAllSelected = false;
					/**Daha önceden eklenmiş idleri array'den çıkarıyoruz*/
					for (var i = 0; i < data.length; i++) {
						var addedRecord = data[i];
						index = $scope.selectedRows.indexOf(addedRecord.id);
						if (index !== -1) {
					...
					for (var i = 0; i < data.length; i++) {
						var record = data[i];
						index = $scope.selectedRows.indexOf(record.id);

						for (var j = 0; j < record.fields.length; j++) {
							var field = record.fields[j];
							/** index === -1 (daha önceden id eklenmemiştir, index >= 0 olduğunda array'de ilgili id daha önceden eklenmiştir.) yapma sebebimiz, daha önceden listeden tekli olarak seçilmiş olan kaydı
							 * kullanıcı isAllSelected'e tıkladığında tekrardan array'e eklememesi için kullandık*/
							if (field.primary && index === -1) {
								$scope.selectedRows.push(record.id);
							}
						}
					}
