        ...
        function ($mdToast) {
            ...
            var setter = function (values) {
                ...
                if (typeof values === 'string')
                    ...
                    angular.extend(defaultValues, values);
            };
            ...
            function getStackToastMsg() {
                if (!stack.length) {
                    return;
                }
                var toastObj = stack[0];
                $mdToast.show(toastObj).then(function () {
                    stack.shift();
                    getStackToastMsg();
                });
            }
            ...
            function toastType(type) {
                if (defaultValues.template) {
                    return {
                        ...
                        textContent: defaultValues.content,
                        action: defaultValues.actionTxt,
                        position: defaultValues.position,
                        actionKey: defaultValues.actionKey,
                        theme: 'toast-' + type,
                        hideDelay: defaultValues.actionTxt ? 0 : defaultValues.timeout,
            ...
            return {
                success: function (value) {
                    setter(value);
                    toastShow(toastType('success'));
                    return;
                },
                warning: function (value) {
                    setter(value);
                    toastShow(toastType('warning'));
                    return;
                },
                error: function (value) {
                    setter(value);
                    toastShow(toastType('error'));
                    return;
                },
                info: function (value) {
                    setter(value);
                    toastShow(toastType('info'));
                    return;
                }
            }
        }])
        ...
        function ($rootScope, $timeout, $filter, $localStorage, $sessionStorage, $q, $http, config, $cache) {
            return {
                ...
                hasPermission: function (moduleName, operation, record) {
                    ...
                    var config = profileConfigs != null ? profileConfigs['config'] != null ? profileConfigs['config'][module.name] : null : null;

                    if (!config)
                        return false;

                    if (operation === 'modify') {
                        return config['actions']['update']['enable'];
                    }

                    else if (operation === 'remove') {
                        return config['actions']['delete']['enable'];
                    }

                    else if (operation === 'write') {
                        return config['actions']['create']['enable'];
                    }
                    ...
                    if (permission && permission[operation]) {
                        return !((operation === 'modify' || operation === 'remove') && record && (!record.shared_users_edit || record.shared_users_edit.indexOf($rootScope.user.id) === -1) && (record.shared_users && record.shared_users.indexOf($rootScope.user.id) > -1));
...
*/
                ...
                mergePicklists: function (picklistsResponse) {
                    ...
                    const picklistGlobalization = $filter('filter')($rootScope.globalizations, function (globalizaiton) {
                        return globalizaiton.Culture.contains(tenantLanguage + '-');
                    }, true)[0];
                    ...
                    if (picklistsResponse) {
                        for (var i = 0; i < picklistsResponse.length; i++) {
                            ...
                            for (var j = 0; j < picklistResponse.items.length; j++) {
                                const picklistResponseItem = picklistResponse.items[j];
                                if(picklistResponseItem.languages  && !angular.isObject(picklistResponseItem.languages)){
                                    picklistResponseItem.languages = JSON.parse(picklistResponseItem.languages)
                                }
                                const label = $rootScope.getLanguageValue(picklistResponseItem.languages, "label");
                                var picklistItem = {};
                                picklistItem.type = picklistResponse.id;
                                picklistItem.id = picklistResponseItem.id;
                                picklistItem.label = label;
                                // picklistItem.label = {};
                                ...
                                if(picklistResponseItem.languages && picklistResponseItem.languages[picklistGlobalization.Label])
                                    picklistItem.labelStr = picklistResponseItem.languages[picklistGlobalization.Label]['label'];
                                picklistItem.languages = picklistResponseItem.languages;
    ...
    .factory('customScripting', ['$timeout', 'ModuleService', '$http', 'config', '$filter', 'blockUI', 'mdToast', '$mdDialog', function ($timeout, ModuleService, $http, config, $filter, blockUI, mdToast, $mdDialog) {
        return {
            run: function (scope, customScript, ev) {
    ...
    .factory('components', ['$rootScope', '$timeout', '$filter', '$localStorage', '$sessionStorage', '$q', '$http', 'config', '$cache', '$injector', '$state', '$stateParams', 'helper', 'mdToast', '$mdDialog', 'blockUI',
        function ($rootScope, $timeout, $filter, $localStorage, $sessionStorage, $q, $http, config, $cache, $injector, $state, $stateParams, helper, mdToast, $mdDialog, blockUI) {
            return {
                run: function (place, type, scope, record, field, moduleName) {//Don't remove record and field. It can be used in components.
                    ...
                    if (moduleName && $rootScope.modulus[moduleName] && $rootScope.modulus[moduleName].components) {
                        var components = $filter('filter')($rootScope.modulus[moduleName].components, function (component) {
                            return component.place === place && component.type === type && (component.module_id === $rootScope.modulus[moduleName].id || component.module_id === 0) && !component.deleted
                        }, true);
                    } else {
...
String.prototype.generateRandomKey = function (length) {
    var result = '';
