...
namespace PrimeApps.App.Controllers
{
    ...
    public class AttachController : MvcBaseController
    {
        ...
        public AttachController(ITenantRepository tenantRepository, ITemplateRepository templateRepository)
        {
        ...
        [Route("export_excel")]
        public async Task<FileStreamResult> ExportExcel([FromQuery(Name = "module")]string module)
        {
    ...
    //                        var moduleEntity = await moduleRepository.GetByName(module);
    //                        var fields = moduleEntity.Fields.OrderBy(x => x.Id).ToList();
    //                        var nameModule = user.culture.Contains("tr") ? moduleEntity.LabelTrPlural : moduleEntity.LabelEnPlural;
    //                        byte[] bytes = System.Text.Encoding.GetEncoding("Cyrillic").GetBytes(nameModule);
    //                        var moduleName = System.Text.Encoding.ASCII.GetString(bytes);
    //                        Workbook workbook = new Workbook(FileFormatType.Xlsx);
    //                        Worksheet worksheetData = workbook.Worksheets[0];
    ...
    //                        workbook.Save(System.Web.HttpContext.Current.Response, moduleName + ".xlsx", ContentDisposition.Attachment, new OoxmlSaveOptions());
    //                        Response.End();
