                ...
                using (var _settingRepository = new SettingRepository(databaseContext, _configuration))
                {
                    ...
                    foreach (var process in processes)
                    {
                        ...
                        if (operationType == OperationType.insert)
                        {
                            ...
                            await notification.AddToQueue(appUser.TenantId, module.Id, (int)record["id"], appUser: appUser);
                        }
                ...
                using (var _settingRepository = new SettingRepository(databaseContext, _configuration))
                {
                    ...
                    if ((process.Approvers.Count != request.ProcessStatusOrder && process.ApproverType == ProcessApproverType.StaticApprover) || (process.ApproverType == ProcessApproverType.DynamicApprover && request.ProcessStatusOrder == 1 && process.ApproverField.Split(',').Length > 1 && !record["custom_approver_2"].IsNullOrEmpty()))
                    {
                        ...
                        if (process.Module.Name == "timetrackers")
                        {
                            ...
                            var timetrackerRecord = (await _recordRepository.Find("timetrackers", findTimetracker))[0];
                        ...
                        else if (process.Module.Name == "masraflar" && !oldExpense)
                        {
                            ...
                            var expenseRecord = (await _recordRepository.Find("masraflar", findExpense))[0];
                        ...
                        if (request.OperationType == OperationType.insert)
                        {
                            var notification = new Email(EmailResource.ApprovalProcessCreateNotification, appUser.Culture, emailData, _configuration, _serviceScopeFactory, appUser.AppId, appUser);
                            notification.AddRecipient(user.Email);
                            await notification.AddToQueue(appUser.TenantId, process.Module.Id, request.RecordId, appUser: appUser);
                        }
                    ...
                    else
                    {
                        ...
                        if (!record["custom_approver_" + processOrder].IsNullOrEmpty())
                        {
                            ...
                            if (request.OperationType == OperationType.insert)
                            {
                                var notification = new Email(EmailResource.ApprovalProcessCreateNotification, appUser.Culture, emailData, _configuration, _serviceScopeFactory, appUser.AppId, appUser);
                                notification.AddRecipient(user.Email);
                                await notification.AddToQueue(appUser.TenantId, process.Module.Id, request.RecordId, appUser: appUser, cc: beforeCc);
                            }
                            else if (request.OperationType == OperationType.update)
                            {
                                var notification = new Email(EmailResource.ApprovalProcessUpdateNotification, appUser.Culture, emailData, _configuration, _serviceScopeFactory, appUser.AppId, appUser);
