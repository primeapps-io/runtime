    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //LDAP login enable control only for application login
                var useLDAP = vm.ApplicationInfo.ApplicationSetting.Options["enable_ldap"];

                if (!studioUrl.Contains(vm.ApplicationInfo.Domain) && !useLDAP.IsNullOrEmpty() && (bool)useLDAP)
                {
        ...
        private async void DatabaseRollback(ApplicationUser user, bool newIdentityUser, bool newPlatformUser, IPlatformUserRepository platformUserRepository, ITenantRepository tenantRepository, Tenant tenant = null, PlatformUser platformUser = null)
        {
            ...
            else if (!newPlatformUser && tenant != null)
        ...
        private async Task<JObject> CreateUser(RegisterInputModel model, ApplicationInfoViewModel applicationInfo, string returnUrl, bool externalLogin = false)
        {
            ...
            else if (identityUser != null && applicationInfo.ApplicationSetting.RegistrationType == RegistrationType.Studio)
            {
            ...
            if (applicationInfo != null)
            {
                ...
                else
                {
                    ...
                    {
                        settings.Culture = applicationInfo.ApplicationSetting.Culture;
                        settings.Currency = applicationInfo.ApplicationSetting.Culture == "en" ? "USD" : "TRY";
                        settings.Language = applicationInfo.Language;
                        settings.TimeZone = applicationInfo.ApplicationSetting.TimeZone;
                    }

                    platformUser = await _userHelper.CreatePlatformUser(userModel, applicationInfo.Name, false, settings);
