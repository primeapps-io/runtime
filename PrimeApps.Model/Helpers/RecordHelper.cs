...
namespace PrimeApps.Model.Helpers
{
    public static class RecordHelper
    {
        ...
        public static async Task<JObject> FieldPermissionControlAsync(Module module, Profile profile, JObject record, IModuleRepository moduleRepository)
        {
        
        //TODO operator parametre eklencek.
            profile.Permissions = profile.Permissions.Where(q => q.Type == EntityType.Module).ToList();

            //Module read permisson control
            var modulePermission = profile.Permissions.Where(q => q.ModuleId == module.Id && q.Read).FirstOrDefault();

            if (modulePermission == null)
                return null;

            //Module Relations read permission control

            foreach (var relation in module.Relations)
            {
                var relationModulePermission = profile.Permissions.Where(q => q.Module.Name == relation.RelatedModule && q.Read).FirstOrDefault();

                if (relationModulePermission == null)
                {
                    var relationModule = await moduleRepository.GetByNameBasic(relation.RelatedModule);
            ...
            //Module Section read permission control
            foreach (var section in module.Sections)
            {
                if (section.Permissions != null && section.Permissions.Count > 0)
                {
                    var sectionPermission = section.Permissions.Where(q => q.ProfileId == profile.Id && q.Type == SectionPermissionType.None && !q.Deleted).FirstOrDefault();
        ...
        private static JObject ClearRecord(JObject record, string key = null, List<string> fields = null)
        {
