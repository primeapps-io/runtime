...
namespace PrimeApps.Model.Helpers
{
    public static class ModuleHelper
    {
        ...
        public static string GenerateTableAlterSql(Module module, ModuleChanges moduleChanges, string language)
        {
            ...
            var indexesDelete = new List<string>();
            ...
            //#3782
            if (moduleChanges.FieldsDeleted.Count > 0)
            {
                indexesDelete = GenerateIndexesDeleteSql(moduleChanges.FieldsDeleted, module.Name, module.Fields);
            }
            ...
            var indexesDeleteSql = string.Join("\n", indexesDelete);
            ...
            if (!(string.IsNullOrEmpty(viewDropCreateSql) && string.IsNullOrEmpty(tableAlterSql) && string.IsNullOrEmpty(junctionTablesSql)
                  && string.IsNullOrEmpty(viewCreateSql) && string.IsNullOrEmpty(indexesCreateSql) && string.IsNullOrEmpty(indexesDeleteSql) && string.IsNullOrEmpty(indexesDropSql)))
                sql = string.Join("\n\n", viewDropCreateSql, tableAlterSql, junctionTablesSql, viewCreateSql, indexesCreateSql, indexesDeleteSql, indexesDropSql);
        ...
        public static Module GetFakeProfileModule(string language)
        {
            var profileModule = new Module();
            profileModule.Name = "profiles";
            profileModule.Fields = new List<Field>();
            profileModule.Fields.Add(new Field {DataType = DataType.TextSingle, Name = "name_" + language, Primary = true});
        ...
        //#3782
        private static List<string> GenerateIndexesDeleteSql(IEnumerable<Field> fields, string moduleName, IEnumerable<Field> moduleFields)
        {
            var indexes = new List<string>();
            ...
            foreach (var field in fields)
            {
                ...
                {
                    indexes.Add(string.Format(IndexDropTemplate, moduleName + "_ix_" + field.Name));
                }
                ...
                if (field.Validation != null && field.Validation.Unique.HasValue && field.Validation.Unique.Value)
                {
                    ...
                    indexes.Add(string.Format(IndexDropTemplate, moduleName + "_ix_unique_" + indexFieldName));
                }
            }
