...
namespace PrimeApps.Model.Repositories
{
    public class TenantRepository : RepositoryBasePlatform, ITenantRepository
    {
        ...
        public async Task<IList<int>> GetIdsByAppId(int appId, bool activeLicense = true)
        {
            var tenantIdsQuery = DbContext.Tenants
                .Include(x => x.License)
                .Where(x => x.AppId == appId && !x.Deleted);

            if (activeLicense)
                tenantIdsQuery = tenantIdsQuery.Where(x => !x.License.IsDeactivated);

            tenantIdsQuery = tenantIdsQuery.OrderBy(x => x.Id);
            var tenantIds = await tenantIdsQuery.Select(x => x.Id).ToListAsync();
