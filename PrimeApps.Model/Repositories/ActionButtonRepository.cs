...
namespace PrimeApps.Model.Repositories
{
	public class ActionButtonRepository : RepositoryBaseTenant, IActionButtonRepository
	{
		...
		{
			var count = await DbContext.ActionButtons
		...
		{
			var actionButtons = GetPaginationGQuery(paginationModel, id)
			...
			if (paginationModel.OrderColumn != null && paginationModel.OrderType != null)
			{
				...
				{
					actionButtons = actionButtons.OrderBy(x => propertyInfo.GetValue(x, null)).ToList();
				}
				else
				{
					actionButtons = actionButtons.OrderByDescending(x => propertyInfo.GetValue(x, null)).ToList();
				}
		...
		private IQueryable<ActionButton> GetPaginationGQuery(PaginationModel paginationModel, int id)
		{
			return DbContext.ActionButtons
				 .Where(x => !x.Deleted && x.ModuleId == id)
